...
void customAnalogWrite(uint8_t channel, uint8_t dutyCycle) {
  ledcWrite(channel, dutyCycle); // Writes the duty cycle to the specified channel
}
...
// PWM channels
const int pwmChannelMotor1 = 0; // Using channel 0 for motor 1
const int pwmChannelMotor2 = 1; // Using channel 1 for motor 2
const int pwmFrequency = 5000;  // 5 kHz as the PWM frequency
...
void setup() {
  ...
  ledcSetup(pwmChannelMotor1, pwmFrequency, pwmResolution);
  ledcSetup(pwmChannelMotor2, pwmFrequency, pwmResolution);
  
  // Attach the motor pins to the PWM channels
  ledcAttachPin(motor1Pin1, pwmChannelMotor1);
  ledcAttachPin(motor2Pin2, pwmChannelMotor2);
...
// Function to adjust speed based on ultrasonic sensor readings
void dynamicSpeedAdjustment() {
  ...
  int speedAdjustmentFactor = 55; // Adjust based on testing

  // Adjust speed based on distance to closest obstacle
  long minDistance = min(distance1, distance2);
  int speedAdjustment = map(minDistance, 0, obstacleThreshold, speedAdjustmentFactor, 0);
  speedAdjustment = constrain(speedAdjustment, 0, speedAdjustmentFactor);

  int leftMotorSpeed = baseSpeed - speedAdjustment;
  int rightMotorSpeed = baseSpeed - speedAdjustment;

  // Further adjust based on hall sensor count to maintain straight line
  if (hallCount1A > hallCount2A) {
    rightMotorSpeed += (hallCount1A - hallCount2A);
  } else if (hallCount2A > hallCount1A) {
    leftMotorSpeed += (hallCount2A - hallCount1A);
  }

  // Apply the adjusted speeds
  customAnalogWrite(motor1Pin1, leftMotorSpeed);
  customAnalogWrite(motor2Pin2, rightMotorSpeed);
}

// Function for differential steering
void differentialTurn(bool shouldTurnLeft, int turnSharpness) {
  int baseSpeed = 200; // Base speed for straight movement
  int turnSpeed = map(turnSharpness, 0, 100, 0, baseSpeed); // Map turn sharpness to speed

  if (shouldTurnLeft) {
    Serial.println("Differential turning left");
    customAnalogWrite(motor1Pin1, baseSpeed - turnSpeed); // Reduce left motor speed
    customAnalogWrite(motor2Pin2, baseSpeed); // Keep right motor at base speed
  } else {
    Serial.println("Differential turning right");
    customAnalogWrite(motor1Pin1, baseSpeed); // Keep left motor at base speed
    customAnalogWrite(motor2Pin2, baseSpeed - turnSpeed); // Reduce right motor speed
  }
}

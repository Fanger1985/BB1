...
def update_position(direction, move_distance):
    ...
    direction_to_offset = {'forward': (0, move_distance), 'backward': (0, -move_distance),
                           'left': (-move_distance, 0), 'right': (move_distance, 0)}
    offset = direction_to_offset.get(direction, (0,0))
    global current_position
    current_position = (current_position[0] + offset[0], current_position[1] + offset[1])
    logging.info(f"Updated position to {current_position}")

def update_map(position, data):
    # Update the environment map with new data at the current position
    environment_map[position] = data
    logging.info(f"Map updated at position {position} with data: {data}")

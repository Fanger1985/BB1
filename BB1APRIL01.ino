...
// Ultrasonic Sensor 2
const int triggerPin2 = 4;  // Changed from 12 to 4 to avoid boot issues REAR SENSOR
const int echoPin2 = 13;    // Echo pin REAR SENSOR
...
void loop() {
  ...
  // Print ultrasonic distances in inches to the serial monitor
  long distance1Inches = readUltrasonicDistanceInInches(triggerPin1, echoPin1);
  long distance2Inches = readUltrasonicDistanceInInches(triggerPin2, echoPin2);
  Serial.print("Distance 1 (inches): ");
  Serial.println(distance1Inches);
  Serial.print("Distance 2 (inches): ");
  Serial.println(distance2Inches);
  ...
  if (action < 5) {  // 5% chance to enter Scout Mode
    ...
    if (distance1Inches < obstacleThreshold || distance2Inches < obstacleThreshold) {
      engageObstacleAvoidance(distance1Inches, distance2Inches);
    } else {

...
async def control_robot(action):
    await send_http_get(action)
    if action in ["forward", "left", "right"]:
        wiggle_servos()  # Wiggle servos during specific actions
...
# Function to "run away" when encountering specific objects
async def run_away():
...
# Function to "follow" based on detected objects
async def follow():

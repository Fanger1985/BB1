...
void detectClientType() {
    ...
    if (userAgent.indexOf("Mobi") != -1 || userAgent.indexOf("Android") != -1) {
        Serial.println("Mobile client detected");
        // Set mobile-specific behavior or flags
    } else {
        Serial.println("Desktop client detected");
        // Set desktop-specific behavior or flags
    }
}
void setup() {
...
// Call this function within each HTTP handler
server.on("/", HTTP_GET, []() {
    detectClientType();
    server.send(200, "text/html", controlPage());
});
...
server.on("/start_tracking", HTTP_GET, []() {
    // Assume start_tracking is called when PIR detects movement
    Serial.println("Tracking mode activated.");
    // Further actions can be triggered here, like enabling camera feed
...
server.on("/sensors", HTTP_GET, []() {
    ...
    doc["ir_left"] = IR_Left_Value;
    doc["ir_right"] = IR_Right_Value;
    ...
    server.on("/gyro", HTTP_GET, []() {
        server.send(200, "application/json", getGyroData());
    });
...
// Function to trigger an emotion on a listening ESP32
void triggerEmotion(String emotion) {
    HTTPClient http;
    const char* listenerIP = "192.168.1.3";  // IP address of the listening ESP32
    String endpoint = "/expressEmotion";  // Endpoint on the listener
    String serverUrl = "http://" + String(listenerIP) + endpoint;  // Complete URL

    http.begin(serverUrl);  // Start the HTTP session
    http.addHeader("Content-Type", "application/json");  // Set the content type

    // Create a JSON object with the specified emotion
    DynamicJsonDocument doc(256);
    doc["emotion"] = emotion;

    String jsonData;
    serializeJson(doc, jsonData);  // Convert to JSON string
...
// Update behavior based on score and trigger appropriate emotion
void adjustBehaviorBasedOnScore() {
    if (score < -10) {  // Low score, trigger "mad" mood
        Serial.println("Score is very low. Robot is mad.");
        triggerEmotion("mad");  // Trigger "mad" on the listener
        cautiousApproach();  // Apply extra cautious approach
    } else if (score < 0) {  // Score is low but not mad level
        Serial.println("Score is low. Robot is grumpy.");
        triggerEmotion("sad");  // Trigger "sad"
        cautiousApproach();  // Cautious approach
    } else if (score >= 10) {  // High score, trigger "happy"
        Serial.println("High score! Robot is happy.");
        triggerEmotion("happy");  // Trigger "happy"
        idleWander();  // Free wandering
    } else {  // Moderate score, could lead to "bored"
        Serial.println("Score is moderate. Robot might be bored.");
        triggerEmotion("bored");  // Trigger "bored"
        cautiousApproach();  // Maintain cautious behavior
    }
}
